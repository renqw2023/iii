...
class Config {
  ...
  get server() {
    return {
      ...
      trustProxy: this.getTrustProxyConfig(),
  ...
  getTrustProxyConfig() {
    ...
    // 数字处理（代理层数）
    if (!isNaN(trustProxy)) return parseInt(trustProxy);
  ...
  get rateLimit() {
    return {
      ...
      legacyHeaders: false,
      // 添加验证配置以解决 ERR_ERL_UNEXPECTED_X_FORWARDED_FOR 错误
      validate: {
        xForwardedForHeader: false, // 禁用 X-Forwarded-For 头部验证
        trustProxy: false, // 禁用 trust proxy 验证
        ip: false, // 禁用 IP 地址验证
        positiveHits: true, // 保持正数命中验证
        singleCount: true, // 保持单次计数验证
        unsharedStore: true // 保持独立存储验证
      },
      // 跳过静态资源和健康检查的限流
      skip: (req) => {
  ...
  get upload() {
    ...
    return {
      ...
      directories: {
        images: path.join(uploadPath, 'images'),
        videos: path.join(uploadPath, 'videos'),
        thumbnails: path.join(uploadPath, 'thumbnails'),
        temp: path.join(uploadPath, 'temp'),
      },
    };
  }
  ...
  get email() {
    ...
    return {
      ...
      from: {
        name: process.env.EMAIL_FROM_NAME || 'MJ Gallery',
  ...
  get logging() {
    return {
      ...
      file: {
        ...
        maxFiles: parseInt(process.env.LOG_FILE_MAX_FILES) || 9,
      },
    };
  }
