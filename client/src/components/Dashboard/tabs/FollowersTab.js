  ...
  if (followers.length === 0) {
    return (
      ...
      >
        ...
        >
          {t('dashboard.followers.empty.action')}
        </a>
      </motion.div>
    );
  }
  ...
  return (
    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
      {followers.map((followerItem, index) => {
        const user = followerItem.follower;
        ...
        return (
          ...
          >
            ...
            <div className="grid grid-cols-3 gap-4 mb-4 text-center">
              <div>
                ...
                <div className="text-xs text-slate-500">
                  {t('dashboard.followers.stats.posts')}
                </div>
              </div>
              <div>
                <div className="text-lg font-semibold text-slate-900">
                ...
                <div className="text-xs text-slate-500">
                  {t('dashboard.followers.stats.followers')}
                </div>
              </div>
              <div>
                <div className="text-lg font-semibold text-slate-900">
                ...
                <div className="text-xs text-slate-500">
                  {t('dashboard.followers.stats.likes')}
                </div>
              </div>
            </div>
            ...
            <div className="text-xs text-slate-400 mb-4">
              {t('dashboard.followers.followedAt')}: {new Date(followerItem.createdAt).toLocaleDateString()}
            </div>
            ...
            <div className="flex space-x-2">
              ...
              >
                {t('dashboard.followers.actions.viewProfile')}
              </a>
              <button
                className="btn btn-outline btn-sm px-3"
                title={t('dashboard.followers.actions.followBack')}
              >
                <UserPlus className="w-4 h-4" />
              </button>
              <button
                className="btn btn-outline btn-sm px-3"
                title={t('dashboard.followers.actions.message')}
              >
