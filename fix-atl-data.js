...
const PromptPost = require('./server/models/PromptPost');
...
// 动态确定PromptPost模型路径
let PromptPostPath;
if (fs.existsSync('./models/PromptPost.js')) {
  PromptPostPath = './models/PromptPost';
} else if (fs.existsSync('./server/models/PromptPost.js')) {
  PromptPostPath = './server/models/PromptPost';
} else {
  console.error('找不到PromptPost模型文件');
  process.exit(1);
}

delete require.cache[require.resolve(PromptPostPath)];
const PromptPostModel = require(PromptPostPath);
...
const fixATLData = async () => {
  try {
    ...
    // 查找所有提示词
    const prompts = await PromptPostModel.find({});
    ...
    // 验证修复结果
    console.log('\n验证修复结果...');
    const verifyPrompts = await PromptPostModel.find({ 'media.0': { $exists: true } }).limit(5);
