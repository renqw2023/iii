...
在手动导入基础上，新增了 **自动同步服务**，随服务器启动自动运行，定期从 meigen.ai 拉取最新 GPT Image 内容。

### 新增/修改文件

| 文件 | 说明 |
|------|------|
| `server/services/syncGptImage.js` | [NEW] 自动同步服务模块 |
| `server/index.js` | [MODIFY] 在服务器启动后调用 `startGptImageSync()` |

### 工作机制

1. **启动延迟**：服务器启动 30 秒后执行首次同步（避免阻塞启动过程）
2. **周期同步**：之后每 24 小时自动同步一次
3. **增量模式**：
   - 已存在的图片文件自动跳过（不重复下载）
   - 数据库使用 `findOneAndUpdate` + `upsert`（已有记录更新，新记录插入）
4. **非阻塞**：同步在后台异步执行，不影响正常 API 服务
5. **错误容错**：单页请求失败不中断整体同步流程

### 服务器日志输出

```
[GPT-Image-Sync] 📅 自动同步已启用 | 首次: 30s 后 | 周期: 24h
[GPT-Image-Sync] 🔄 开始同步 meigen.ai GPT Image 数据...
[GPT-Image-Sync]   📊 共抓取 323 条 GPT Image 数据 (54 页)
[GPT-Image-Sync] ✅ 同步完成 (438.2s) | 图片: +0 ⏭580 ❌0 | 数据: +323 🔄0 ❌0
```

### 验证结果

- ✅ 服务器启动后 30s 自动开始首次同步
- ✅ 增量模式正常：580 张已有图片全部跳过
- ✅ 同步过程不影响 API 服务正常响应
- ✅ 同步完成后自动安排下次 24h 后执行
